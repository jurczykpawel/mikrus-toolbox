...
const CONFIG_DIR = join(homedir(), ".config", "sellf");
...
export const setupSellfTool = {
  name: "setup_sellf_config",
  description:
...
export async function handleSetupSellf(
  ...
  if (existsSync(CONFIG_PATH) && !projectRef && !verificationCode) {
    return {
      content: [{
        type: "text",
        text: `Sellf config already exists at ${CONFIG_PATH}.\n` +
          `To reconfigure, delete the file and run this tool again.\n` +
          `You can now deploy with: deploy_app(app_name="sellf")`,
      }],
    };
  }
  ...
  if (projectRef) {
    ...
    const configContent = [
      ...
      `# Generated by MCP setup_sellf_config`,
      ``,
    ...
    return {
      content: [{
        type: "text",
        text: `Sellf configuration saved to ${CONFIG_PATH}\n\n` +
          `Project: ${projectRef}\n` +
          `Supabase URL: https://${projectRef}.supabase.co\n\n` +
          `You can now deploy with: deploy_app(app_name="sellf", domain_type="cytrus", domain="auto")`,
      }],
    };
  }
  ...
  if (verificationCode) {
      ...
      return {
        ...
        content: [{
          ...
          text: `Failed to get token. The verification code may be incorrect or expired.\n` +
            `Start over by calling setup_sellf_config() again.`,
        }],
      };
    }
    ...
    if (projects.length === 1) {
      ...
      return handleSetupSellf({ project_ref: projects[0].id });
    }
    ...
    return {
      content: [{
        ...
        text: `Supabase login successful! Found ${projects.length} projects:\n\n` +
          `${projectList}\n\n` +
          `Ask the user which project to use for Sellf, then call:\n` +
          `setup_sellf_config(project_ref="<selected_project_id>")`,
      }],
    };
  }
  ...
  if (existingToken) {
    ...
    if (Array.isArray(testResponse)) {
      ...
      if (testResponse.length === 1) {
        return handleSetupSellf({ project_ref: testResponse[0].id });
      }
  ...
  return {
    content: [{
      ...
      text: `Opening browser for Supabase login...\n\n` +
        ...
        `setup_sellf_config(verification_code="<the_code>")`,
    }],
  };
}
